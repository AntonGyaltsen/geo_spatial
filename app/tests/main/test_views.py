...
def test_get_polygon_by_id(client):
    ...
    assert response_data['properties'][
               'name'] == 'Test Polygon', f"Expected name 'Test Polygon', but got {response_data['properties']['name']}"

    geometry = response_data['geometry']
    assert 'POLYGON' in geometry, f"Expected geometry to be a POLYGON, but got {geometry}"

    stored_coords = polygon.polygon.coords[
        0]
    assert stored_coords == coordinates, (f"Expected coordinates"
                                          f" {coordinates}, but got"
                                          f" {stored_coords}")

    assert response_data['properties']['crosses_antimeridian'] is True, \
...
def test_delete_polygon(client):
    ...
    polygons = PolygonModel.objects.all()
    assert len(
        polygons) == 0, "The polygon should have been deleted, but it's still in the database"
...
def test_update_polygon(client):
    ...
    assert polygon.name == 'Updated Polygon', f"Expected name 'Updated Polygon', but got {polygon.name}"
    assert polygon.polygon.coords[0] == updated_coordinates, \
        f"Expected coordinates {updated_coordinates}, but got {tuple(polygon.polygon.coords[0])}"
    assert polygon.crosses_antimeridian is False, \
